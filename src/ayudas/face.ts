import { BufferGeometry, BufferAttribute, Vector3, Triangle, Matrix4 } from 'three/src/Three.js';
import { FACES as indices, UVS as texCoords } from '../ayudas/geometry';
//import { BufferGeometry, BufferAttribute, Vector3, Triangle, Matrix4 } from 'three-js';
//import { setAttribute, setIndex, computeVertexNormals, applyMatrix4, getAttribute } from 'three-js';
...
class FaceMeshFaceGeometry extends BufferGeometry {
  ...
  constructor(options = { useVideoTexture: false, normalizeCoords: false }) {
    super();
  ...
  setVideoUvs() {
    ...
    for (let j = 0; j < 468 * 2; j += 2) {
      this.uvs[j] = this.flipped ? this.positions[ptr] / this.w + 0.5 : 1 - (this.positions[ptr] / this.w + 0.5);
  ...
  update(face, cameraFlipped) {
    ...
    for (const p of face.scaledMesh) {
      this.positions[ptr] = cameraFlipped ? p[0] + 0.5 * this.w : p[0] - 0.5 * this.w;
