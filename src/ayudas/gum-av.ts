...
export default class GumAudioVideo extends HTMLElement {
  deviceNameLabel;
  nextDeviceButton;
  currentVideoInput;
  devices;
  ...
  invalidateVideoSource() {
    this.videoLoadedData = new Promise((resolve, reject) => {
      try {
        ...
        console.log('no hay datos de video');
      }
    });
  }
  ...
  async getMedia(device) {
    ...
    var streamVideo = video.captureStream();
    ...
    try {
      stream = streamVideo; //await navigator.mediaDevices.getUserMedia(constraints);
